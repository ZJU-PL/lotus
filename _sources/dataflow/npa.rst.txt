...
* **Adapters/LLVM/**: LLVM bit-vector and interprocedural integration. Interprocedural
...
* **Tooling/**: Drivers for NPA execution, notably ``NPALLVMDriver`` for LLVM IR clients 
  and ``NPABooleanDriver`` for Boolean programs.
...
* **Adapters/LLVM/IntraEngine.h**: Intraprocedural equation construction for the same
...
In addition to LLVM bit-vector clients, NPA supports the analysis of **Boolean programs** 
via the ``NPABooleanDriver`` (see ``include/Dataflow/NPA/Tooling/NPABooleanDriver.h``). 
Boolean programs are often generated by CEGAR loops and represent abstracted state spaces. 

For this purpose, NPA supports the **Predicate Abstraction Domain** (``PredicateRelationDomain``) 
to capture symbolic transitions and applies Newtonian iteration to solve reachability and safety properties. 
Boolean program parsing and lowering are handled by the ``Verification/Frontend`` subsystem, 
translating them directly into NPA equation systems for resolution.
